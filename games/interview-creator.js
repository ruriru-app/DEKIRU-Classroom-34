...
(()=>{
 ...
 function readSentences(){return [...root.querySelectorAll('[data-sentence-input]')].map(e=>e.value);}
 function sentences(values){const box=root.querySelector('#interviewSentences');box.replaceChildren();values.forEach((value,i)=>{const row=document.createElement('div');row.className='interview-row';row.innerHTML=`<label class="interview-field">${i+1}文目<input data-sentence-input ${i===0?'id="interviewQuestion"':''} maxlength="200" value="${esc(value)}"></label><button type="button" aria-label="${i+1}文目を削除" ${values.length===1?'disabled':''}>−</button>`;row.querySelector('button').onclick=()=>sentences(readSentences().filter((_,n)=>n!==i));box.append(row);});}
 function open(id){try{editing=id?store().listPresets().find(p=>p.id===id):null;if(id&&!editing)throw Error('プリセットが見つかりません');
  root.innerHTML='<div class="head"><button type="button" aria-label="戻る" id="interviewAuthorBack">戻る</button><h1>Interviewを作る</h1></div><p>このブラウザに保存</p><p>本ページへの公開は別途依頼してください。端末間では自動同期されません。</p><div class="interview-form">'+
   '<button type="button" id="interviewColors">Colorsサンプルを作成</button>'+field('プリセット名','interviewName',80,editing?.name)+field('タイトル','interviewTitle',80,editing?.title||'INTERVIEW')+field('説明','interviewDescription',500,editing?.description)+'<section><h2>質問文</h2><div id="interviewSentences"></div><button type="button" id="interviewAddSentence" aria-label="文を追加">＋</button><small>1つの欄に1文を入力します。全体で200文字以内です。</small></section>'+
  ...
  sentences(M.sentenceTemplates(editing?.question.template||'Do you like (P)?'));
  root.querySelector('#interviewAddSentence').onclick=()=>{sentences([...readSentences(),'']);[...root.querySelectorAll('[data-sentence-input]')].at(-1).focus();};
  root.querySelector('#interviewCards').innerHTML=[...groups].map(([category,cards])=>`<details data-card-category="${esc(category)}"><summary><span>${esc(categoryLabel(category))}</span><button type="button" class="interview-category-toggle" data-category-toggle="${esc(category)}" aria-label="${esc(categoryLabel(category))}をまとめて選択・解除">まとめて選択</button></summary><div class="interview-options">${cards.map(c=>`<button type="button" class="interview-chip" data-card-id="${esc(c.id)}" aria-pressed="${selected.has(c.id)}">${esc(c.english)}</button>`).join('')}</div></details>`).join('');
  function count(){root.querySelector('#interviewCardCount').textContent=selected.size+'枚を選択中';root.querySelectorAll('[data-card-id]').forEach(e=>e.setAttribute('aria-pressed',String(selected.has(e.dataset.cardId))));root.querySelectorAll('[data-category-toggle]').forEach(e=>{const list=groups.get(e.dataset.categoryToggle),n=list.filter(c=>selected.has(c.id)).length;e.setAttribute('aria-pressed',n===list.length?'true':n?'mixed':'false');e.textContent=n===list.length?'まとめて解除':'まとめて選択';});}count();
  root.querySelectorAll('[data-card-id]').forEach(e=>e.onclick=()=>{const id=e.dataset.cardId;selected.has(id)?selected.delete(id):selected.add(id);count();});
  root.querySelectorAll('[data-category-toggle]').forEach(e=>e.onclick=event=>{event.preventDefault();event.stopPropagation();const list=groups.get(e.dataset.categoryToggle),all=list.every(c=>selected.has(c.id));list.forEach(c=>all?selected.delete(c.id):selected.add(c.id));count();});
  ...
  root.querySelector('#interviewColors').onclick=()=>{try{
   ...
   editing=null;root.querySelector('#interviewName').value='Colors';root.querySelector('#interviewTitle').value='INTERVIEW';root.querySelector('#interviewDescription').value='色について友達に質問しよう';sentences(['Do you like (P)?']);
   selected.clear();ids.forEach(id=>selected.add(id));root.querySelectorAll('[name=interviewUnit]').forEach(e=>e.checked=e.value==='lt1:4');
  ...
  root.querySelector('#interviewSave').onclick=()=>{try{
   const now=new Date().toISOString(),cardIds=rouletteAvailableCards().filter(c=>selected.has(c.id)).map(c=>c.id),template=readSentences().flatMap(M.sentenceTemplates).join('\n');
 ...
 function renderLibrary(){const library=document.getElementById('interviewLibrary');library.replaceChildren();try{const all=store().listPresets();if(!all.length){library.textContent='保存したInterviewはありません。';return;}
  all.forEach(p=>{const article=document.createElement('article');article.className='created-game-card interview-saved-card';article.innerHTML=`<div class="interview-saved-info"><strong>${esc(p.name)}</strong><span>${esc(p.title)}</span><small>このブラウザ内のみ・${p.cardIds.length}枚／${p.answerAreas.length}分類</small></div><div class="interview-saved-actions"><button type="button" data-edit>編集</button><button type="button" data-delete>削除</button><a href="../grade34/interview.html?preset=${encodeURIComponent(p.id)}&source=local">教師画面で試す</a><button type="button" data-download>設定ファイルDL</button></div>`;
   article.querySelector('[data-edit]').onclick=()=>open(p.id);article.querySelector('[data-download]').onclick=()=>{const clean=M.validatePreset(p),url=URL.createObjectURL(new Blob([JSON.stringify(clean,null,2)],{type:'application/json'}));const link=document.createElement('a');link.href=url;link.download=p.id+'.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};article.querySelector('[data-delete]').onclick=()=>{if(!confirm('このプリセットを削除しますか？'))return;try{store().deletePreset(p.id);renderLibrary();}catch(e){const error=document.createElement('p');error.setAttribute('role','status');error.textContent=e.message;article.append(error);}};library.append(article);
  });
