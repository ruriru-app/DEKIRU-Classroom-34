...
(()=>{
 ...
 function open(id){try{editing=id?store().listPresets().find(p=>p.id===id):null;if(id&&!editing)throw Error('プリセットが見つかりません');
  ...
  root.querySelector('#interviewAuthorBack').onclick=async()=>{if(document.fullscreenElement)await document.exitFullscreen();else{renderLibrary();show('createGames');}};
  ...
  root.querySelector('#interviewSave').onclick=()=>{try{
   ...
   const value={version:1,type:'interview',id:editing?.id||M.newId(),name:root.querySelector('#interviewName').value,title:root.querySelector('#interviewTitle').value,description:root.querySelector('#interviewDescription').value,assignedUnits:[...root.querySelectorAll('[name=interviewUnit]:checked')].map(e=>{const [bookId,n]=e.value.split(':');return {bookId,unit:Number(n)};}),question:{template,slots:template.includes('(P)')?[{id:'P',type:'picture-card',cardIds}]:[]},cardIds,answerAreas:readAnswers(),createdAt:editing?.createdAt||now,updatedAt:now};
   store().savePreset(M.validatePreset(value,new Set(rouletteAvailableCards().map(c=>c.id))));renderLibrary();show('createGames');
  }catch(e){status(e.message);}};
  show('createInterview');
 }catch(e){document.getElementById('interviewLibrary').textContent=e.message;show('createGames');}}
