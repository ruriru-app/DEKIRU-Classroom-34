window.BombGame = (() => {
  ...
  function paint(){
    ...
    if(phase==='idle'||phase==='loading')stage.innerHTML=`<p class="look-message">${phase==='loading'?'準備中…':'STARTを押してください'}</p>`;
      ...
      stage.innerHTML=`<div class="bomb-grid" style="--columns:${grid.columns};--rows:${grid.rows}">${deck.map((item,i)=>{
        ...
        let mark=safe.has(i)?'<span class="bomb-safe-mark" aria-label="SAFE">○</span>':'';
  ...
  function stop(reset=false){cancel();if(reset||busy()){deck=[];safe=new Set();bomb=-1;phase='idle';}}
  ...
  function pick(index){
    ...
    if(index!==bomb){safe.add(index);safeTone();paint();return;}
  ...
  return {markup,settingsMarkup,choose,layout,stop,configure,pick,isRunning:busy,attach(options){host=options;paint();},action(name){if(name==='start')start();}};
})();
