(function(root){
 ...
 function validate(v){
  ...
  const students=rows.map((s,i)=>{const names=cleanNames(s.names);return {id:M.id(s.id),number:nums[i],names,...(s.legacyName?{legacyName:M.text(s.legacyName,'旧氏名',80)}:{})};});
 ...
 function parse(input,previous=[]){
  ...
  if(rows[0]?.line.trim()===headers[0]||rows[0]?.line.split('\t').map(c=>c.trim()).join('\t')===headers.join('\t'))rows.shift();
  M.check(rows.length>=1&&rows.length<=100,'名簿は1〜100人で入力してください');const warnings=[];
  const students=rows.map(({line,i})=>{const c=line.split('\t');if(c.length===1)c.push(...Array(6).fill(''));M.check(c.length===7,i+'行目：番号のみの1列、または7列で貼り付けてください');const names=emptyNames();scripts.forEach((s,k)=>{names[s]={family:c[1+k*2].trim(),given:c[2+k*2].trim()};});return {id:M.newId('student'),number:number(c[0],i+'行目の出席番号'),names:cleanNames(names)};});
  numbers(students);
 ...
 function toDeliveryRoster(value,{script,scope}){
  M.check(['full','given','number'].includes(scope),'表示範囲を選んでください');
  const v=value?.version===1?M.validateRoster(value):validate(value);const nums=numbers(v.students);
  if(scope==='number')return M.validateRoster({version:1,id:v.id,className:v.className,students:v.students.map((s,i)=>({id:s.id,number:nums[i],name:''})),createdAt:v.createdAt,updatedAt:v.updatedAt});
  M.check(v.version!==1||scope==='full','旧形式の名簿は氏名全体または出席番号のみを選んでください');
